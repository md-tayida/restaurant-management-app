...
const MenuPage = () => {
    ...
    useEffect(() => {
        ...
        const loadTables = async () => {
            try {
                ...
                const tableData = Array.isArray(res) ? res : res.data;
                setTables(tableData || []);
            } catch (err) {
    ...
    const handleSubmitOrder = async () => {
        ...
        const orderPayload = {
            ...
            orderItems: selectedItems.map(orderItem => ({
                menuId: orderItem.id,
                name: orderItem.name,
        ...
        const handleDescriptionChange = (menuId, value) => {
            setSelectedItems(prev =>
                prev.map(item =>
                    item.id === menuId ? { ...item, description: value } : item
                )
            );
        };
        ...
        <div style={{ fontFamily: 'Arial, sans-serif' }}>
                    ...
                    <div style={{ marginTop: '10px' }}>
                        ...
                        >
                            ...
                            {tables && tables.length > 0 ? (
                                ...
                                <option disabled>ไม่มีโต๊ะ</option>
                            )}
                            56
                        </select>
                    </div>
            ...
            <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#f1f2f6' }}>
                ...
                <div style={{ flex: 2, padding: '30px', overflowY: 'auto' }}>
                    ...
                    {!loading && !error && Object.keys(menuByCategory).length === 0 && (
                        <p>ยังไม่มีเมนูอาหารในขณะนี้</p>
                    )}
